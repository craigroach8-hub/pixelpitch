...
// Pixel Pitch: assign a random pixel for a given color
// Pixel Pitch: assign a random pixel for a given color
  try {
    ...
    // Log this assignment as an event
    await Assignment.create({
      pixelId: pixel._id,
      color: pixel.color,
    });

    // Respond (still just color + id, same as before)
    return res.json({
      color: pixel.color,
      id: pixel._id,
    });
...
// Pixel Pitch: get recent assignments (activity feed)
...
// Pixel Pitch: return all pixels (for simple canvas view)
